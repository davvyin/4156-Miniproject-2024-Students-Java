...
public class RouteController {
    ...
    try {
      ...
      if (doesDepartmentExists) {
        ...
        coursesMapping = departmentMapping.get(
            deptCode.toUpperCase(Locale.ROOT)).getCourseSelection();
  ...
  public ResponseEntity<?> isCourseFull(@RequestParam("deptCode") String deptCode,
    ...
    try {
      ...
      doesCourseExists = retrieveCourse(deptCode.toUpperCase(Locale.ROOT), courseCode)
          .getStatusCode() == HttpStatus.OK;
  ...
  public ResponseEntity<?> getMajorCtFromDept(@RequestParam("deptCode") String deptCode) {
    try {
      ...
      if (doesDepartmentExists) {
        Map<String, Department> departmentMapping;
        departmentMapping = IndividualProjectApplication.myFileDatabase.getDepartmentMapping();
        return new ResponseEntity<>("There are: " + departmentMapping
    ...
    try {
      ...
      if (doesDepartmentExists) {
        ...
        return new ResponseEntity<>(
            departmentMapping.get(deptCode.toUpperCase(Locale.ROOT)).getDepartmentChair() + " is "
